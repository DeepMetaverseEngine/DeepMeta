...
namespace DeepCore.Voxel.Data
{
    ...
    public class VoxelObject : ITerrainAgent
    {
        private VoxelWorld world;
        private VoxelLayer currentLayer;
        private Vector3 currentPos;
        private float zspeed = 0;
        private float height = 1.8f;
        private float gravity = 10f;

        public VoxelWorld World { get => world; }
        public VoxelTerrain3D Terrain { get => world.Terrain; }
        public VoxelLayer CurrentLayer { get => currentLayer; }
        ...
        public VoxelObject(Vector3 pos)
        {
            this.currentPos = pos;
        }
        public VoxelObject(VoxelLayer pos)
        {
        ...
        public void EnterWorld(VoxelWorld world)
        {
            var oldLayer = currentLayer;
            this.world = world;
            this.currentLayer = Terrain.GetVoxelLayerByObject(ref currentPos);
        ...
        public void Transport(VoxelLayer layer)
        {
            var oldLayer = currentLayer;
        ...
        public void Transport(Vector3 pos, VoxelLayer layer)
        {
            var oldLayer = currentLayer;
            this.currentPos = pos;
            this.currentLayer = layer;
        ...
        /// <returns></returns>
        public AgentMoveResult TryMoveToPath(ref IVoxelWayPoint path, float step, bool land)
        {
        ...
        protected AgentMoveResult TryMoveToNext3D(Vector3 target, bool land)
        {
            ...
            var step = zspeed > 0 ? 0f : world.Terrain.BuildConfig.StepIntercept;
            ...
            if (adx <= 1 && ady <= 1)
            {
                ...
                {
                    //下格无碰撞//
                    if (next_cell.TryStandOn(ref currentPos, IgnoreTop ? 0 : height, step, out var next))
                    {
                        //同色地块或者在空中//
                        if ((!MoveKeepInColor || currentLayer.Color == next.Color) || land == false)
                        {
                            currentLayer = next;
        ...
        /// <returns></returns>
        public AgentMoveResult MoveLinearTo2D(Vector3 dst, out VoxelLayer touched)
        {
            var oldLayer = currentLayer;
            ...
            //                 return true;
            //             });
            Terrain.TryGetVoxelLayerByPos(dst, out var dstCell, out var dstLayer);
            world.Terrain.TryBlinkToTarget2D(currentLayer, currentPos, dstLayer, dst, out currentLayer, out currentPos);
