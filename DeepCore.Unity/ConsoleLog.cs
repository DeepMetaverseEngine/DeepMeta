...
{
    public int FocuseOpenClickCount = 5;
    /// <summary>
    /// log信息
    /// </summary>
    struct LogMsg
    ...
    /// 控制台是否是打开的
    /// </summary>
    static bool isDrawGui = false;
    public static bool IsDrawGui { get => isDrawGui; set => isDrawGui = value; }
    ...
    void OnGUI()
    {
        if (Event.current.isMouse && Event.current.type == EventType.MouseDown && Event.current.clickCount == FocuseOpenClickCount)
        {
            if (isDrawGui)
            {
                isDrawGui = false;
