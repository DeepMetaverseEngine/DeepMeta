...
namespace ST.Library.UI.NodeEditor
{
    ...
    public class STNodePropertyDescriptor
    {
        ...
        protected internal virtual object GetValueFromString(string strText) {
            ...
            if (t == m_t_int) return Parser.ParseInt(strText);
            if (t == m_t_float) return Parser.ParseFloat(strText);
            if (t == m_t_double) return Parser.ParseDouble(strText);
            if (t == m_t_string) return strText;
            if (t == m_t_bool) return Parser.ParseBool(strText);
            if (t.IsEnum) {
                ...
                if (t_1 == m_t_int) {
                    ...
                    for (int i = 0; i < strs.Length; i++) arr[i] = Parser.ParseInt(strs[i].Trim());
                    return arr;
                }
                if (t_1 == m_t_float) {
                    float[] arr = new float[strs.Length];
                    for (int i = 0; i < strs.Length; i++) arr[i] = Parser.ParseFloat(strs[i].Trim());
                    return arr;
                }
                if (t_1 == m_t_int) {
                    double[] arr = new double[strs.Length];
                    for (int i = 0; i < strs.Length; i++) arr[i] = Parser.ParseDouble(strs[i].Trim());
                    return arr;
                }
                if (t_1 == m_t_int) {
                    bool[] arr = new bool[strs.Length];
                    for (int i = 0; i < strs.Length; i++) arr[i] = Parser.ParseBool(strs[i].Trim());
                    return arr;
                }
            }
