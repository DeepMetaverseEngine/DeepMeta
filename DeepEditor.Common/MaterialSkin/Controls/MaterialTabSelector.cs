namespace MaterialSkin.Controls
{
    ...
    public class MaterialTabSelector : Control, IMaterialControl
    {
        ...
        private const int TAB_WIDTH_MIN = 20;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (!_animationManager.IsAnimating() || _tabRects == null || _tabRects.Count != _baseTabControl.TabCount)
                UpdateTabRects();
            if (_baseTabControl.TabCount == 0) return;
            ...
            foreach (TabPage tabPage in _baseTabControl.TabPages)
            {
                ...
                var textColor = SkinManager.ColorScheme.FORM_TITLE_TEXT_COLOR;
                if (_tabLabel != TabLabelStyle.Icon)
                {
                    ...
                    using (NativeTextRenderer NativeText = new NativeTextRenderer(g))
                    {
                        ...
                        if (((TAB_HEADER_PADDING * 2) + textSize.Width < TAB_WIDTH_MAX))
                        {
                            ...
                            Font,
                            Color.FromArgb(CalculateTextAlpha(currentTabIndex, animationProgress), textColor),
                ...
                if (_tabLabel != TabLabelStyle.Text)
                {
                    ...
                    if (_baseTabControl.ImageList != null && (!String.IsNullOrEmpty(tabPage.ImageKey) | tabPage.ImageIndex > -1))
                    {
                        ...
                        if (!string.IsNullOrEmpty(tabPage.ImageKey) && _baseTabControl.ImageList.Images[tabPage.ImageKey] is Image img)
                        {
                            g.DrawImage(img, iconRect);
                        }
                        else if (tabPage.ImageIndex >= 0 && _baseTabControl.ImageList.Images[tabPage.ImageIndex] is Image imgi)
                        {
                            g.DrawImage(imgi, iconRect);
                        }
                    }
                }
            }
