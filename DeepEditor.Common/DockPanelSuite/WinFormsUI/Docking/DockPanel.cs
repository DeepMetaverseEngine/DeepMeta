...
namespace WeifenLuo.WinFormsUI.Docking
{
    ...
    public partial class DockPanel : Panel
    {
        ...
        public void ShowInGroup(DockState dockState, params DockContent[] forms)
        {
            if (forms.Length > 0)
            {
                forms[forms.Length - 1].Show(this, dockState);
                var last = forms[forms.Length - 1];
                for (int i = forms.Length - 2; i >= 0; --i)
                {
                    forms[i].Show(last.PanelPane, last);
                    last = forms[i];
                }
            }
        }
        ...
        [DefaultValue(false)]
        public bool DockLeftFixed { get; set; } = false;
        [DefaultValue(false)]
        public bool DockBottomFixed { get; set; } = false;
        [DefaultValue(false)]
        public bool DockRightFixed { get; set; } = false;
        [DefaultValue(false)]
        public bool DockTopFixed { get; set; } = false;
        ...
        public int GetDockWindowSize(DockState dockState)
        {
            if (dockState == DockState.DockLeft || dockState == DockState.DockRight)
            {
                ...
                if (DockLeftFixed)
                {
                    var left = this.DockWindows[DockState.DockLeft];
                    if (left != null && left.Width > MeasurePane.MinSize)
                    {
                        dockLeftSize = left.Width;
                    }
                }
                if (DockRightFixed)
                {
                    var right = this.DockWindows[DockState.DockRight];
                    if (right != null && right.Width > MeasurePane.MinSize)
                    {
                        dockRightSize = right.Width;
                    }
                }
#endif
            ...
            if (dockState == DockState.DockTop || dockState == DockState.DockBottom)
            {
                ...
                if (DockTopFixed)
                {
                    var top = this.DockWindows[DockState.DockTop];
                    if (top != null && top.Width > MeasurePane.MinSize)
                    {
                        dockTopSize = top.Height;
                    }
                }
                if (DockBottomFixed)
                {
                    var bottom = this.DockWindows[DockState.DockBottom];
                    if (bottom != null && bottom.Width > MeasurePane.MinSize)
                    {
                        dockBottomSize = bottom.Height;
                    }
                }
#endif
