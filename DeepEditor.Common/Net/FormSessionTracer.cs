...
{
    public partial class FormSessionTracer : DeepEditor.Common.G2D.G2DBaseForm
    {
        ...
        public FormSessionTracer(ISession session, int capacity = 600)
        {
            InitializeComponent();

            this.mSession = session;

            this.mTrackerRecv = new BytesSecondRateTracker(capacity, this.Font, new Pen(Color.Blue), new SolidBrush(Color.Blue));
            this.mTrackerRecv.Title = "Recv";
            this.mTrackerSend = new BytesSecondRateTracker(capacity, this.Font, new Pen(Color.Red), new SolidBrush(Color.Red));
            this.mTrackerSend.Title = "Send";
        }

        private void timer1_Tick(object sender, EventArgs e)
        {
            this.Text = mSession.Title;
