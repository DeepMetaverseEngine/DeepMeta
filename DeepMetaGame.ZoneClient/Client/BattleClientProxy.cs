...
namespace DeepMetaGame.ZoneServer.Client
{
    ...
    {
        private IMinaClientSession proxy_session;
        private enum Status : int
        {
            ON_LINE, OFF_LINE,
        }
        private AtomicState<Status> status = new AtomicState<Status>(Status.OFF_LINE, static (a, b) => a == b);

        public override IMinaClientSession Session
        {
            get { return proxy_session; }
        }
        public BattleClientProxy(
            IMinaClientSession netSession,
        ...
        public override void Start()
        {
            if (status.Update(Status.ON_LINE))
            {
                ...
                proxy_session.Send(new ConnectToProxy().Init(room.Room.ClientConnectString));
        ...
        private void proxy_session_OnMessageReceived(IMinaClientSession session, object data)
        {
        ...
        private void proxy_session_OnMessageSent(IMinaClientSession session, object data)
        {
            base.callback_messageSent(session, data);
        }
        private void proxy_session_OnError(IMinaClientSession session, Exception err)
        {
            base.callback_onError(session, err);
        }

        //----------------------------------------------------------------------------------------------------
        class DirectProxyListener : IMinaClientSessionListener
        {
            ...
            public void OnSessionOpened(IMinaClientSession session)
            {
            }
            public void OnSessionClosed(IMinaClientSession session)
            {
            }
            public void OnMessageReceived(IMinaClientSession session, object data)
            {
            }
            public void OnMessageSent(IMinaClientSession session, object data)
            {
            }
            public void OnError(IMinaClientSession session, Exception err)
            {
            }
        }
        ...
        /// </summary>
        public abstract class ProxyNetSession : MinaSocketSession
        {
