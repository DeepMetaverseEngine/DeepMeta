...
namespace DeepCore.Pomelo
{
    public class FixedHeadProtocolDecoding : Disposable
    {
        ...
        private MemoryStream recv = new MemoryStream();
            ...
            while (count > 0)
            {
                ...
                if (readed < fixedHeadSize)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new Exception(string.Format("PkgLength:{0} out of limit:{1} ", pkgLength, this.maxRequestLength));
                        }
                    }
                }
                else
                ...
                while (count > 0)
                {
                    ...
                    if (readed < IRecvMessage.FIXED_HEAD_SIZE)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                throw new Exception(string.Format("PkgLength:{0} out of limit:{1} ", recv.PkgLength, this.maxRequestLength));
                            }
                        }
                    }
                    else
