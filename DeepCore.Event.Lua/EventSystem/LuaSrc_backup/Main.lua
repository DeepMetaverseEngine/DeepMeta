...
------------------------------------------------------------------
-------------------------Use in csharp---------------------------
local function ServerEventLogic(func, lastParent, ...)
    if lastParent == 'Task' or lastParent == 'Listen' then
        local eid = func(...)
        if not eid then
            inner_error('create event failed ' .. get_print_string(true, ...))
        end
        CurrentManager:ListenEvent(
            eid,
            function(...)
...
function CreateServerEventTable(apiName, arg)
    ...
    local func, functype = EventApi.String2Function(apiName)
    if not func then
        error('call error ' .. apiName)
    else
        local eventTable = CreateEventTable(ServerEventLogic, func, functype, ArgTableToDynamic(arg))
        if eventTable then
            return eventTable
        end
    end
