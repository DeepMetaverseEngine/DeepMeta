...
namespace DeepCore.NetClient
{
    public abstract class IProtocolMessage : IProtocol
    {
        ...
        public void FillBuffer(ReadOnlySequence<byte> segment)
        {
            foreach (var sp in segment)
            {
                this.buffer.Write(sp.Span);
            }
        }
        public void FillBuffer(ArraySegment<byte> segment)
        {
            this.buffer.Write(segment);
        }
    }
    ...
    public abstract class ISendMessage : IProtocolMessage
    {
        ...
        {
            this.buffer_stream = ProtocolPool.Instance.CreateOutputStream(buffer, codec);
        ...
        public override Type BodyType
        {
            get
            {
                if (sending_msg != null) return sending_msg.GetType();
                if (sending_sys != null) return sending_sys.GetType();
                return SendingObject?.GetType();
            }
        }
        protected override void Disposing()
        {
        ...
        protected void BeginSend()
        {
            ...
            if (sending_msg != null)
            {
                ...
                if (ProtocolPool.Instance.CompressStream(this.buffer, this))
                {
    ...
    public abstract class IRecvMessage : IProtocolMessage
    {
        ...
        {
            this.buffer_stream = ProtocolPool.Instance.CreateInputStream(buffer, codec);
        ...
        public override Type BodyType
        {
            get
            {
                if (readed_msg != null) return readed_msg.GetType();
                if (readed_sys != null) return readed_sys.GetType();
                return bodyType ?? (bodyType = buffer_stream?.Factory?.GetCodec(this.MsgRoute)?.MessageType);
            }
        }
        protected override void Disposing()
        {
        ...
        public ISerializable ReadBody()
        {
            if (readed_msg == null)
            {
                ...
                try
                {
                    ...
                    {
                        ProtocolPool.Instance.DecompressStream(this.buffer, this);
                    }
                    var codec = buffer_stream.Factory.GetCodec(this.MsgRoute);
                    if (codec != null)
                    {
                        this.bodyType = codec.MessageType;
                        buffer.Position = this.BodyStartPistion;
        ...
        public BinaryMessage ReadBodyBinary()
        {
            if (readed_bin.IsNoRoute)
            {
                ...
                try
                {
                    ...
                    buffer.Position = this.BodyStartPistion;
                    this.bodyType = buffer_stream?.Factory?.GetCodec(this.MsgRoute)?.MessageType;
                    readed_bin = buffer_stream.DecodeBinaryMessage(MsgRoute, bodyType, this.BodyStartPistion, this.BodyLength);
