...
namespace NFCore.Extension
{
    ...
    where U : unmanaged
    {
        ...
        public void Resize(int _capacity)
        {
            ...
            int remainder = _capacity % HudRendererBatch.batchMaxCount;
            int partCount = _capacity / HudRendererBatch.batchMaxCount;
            partCount = remainder == 0 ? partCount : (partCount + 1);
            int addCount = partCount - arrays.Count;
            for (int i = 0; i < addCount; i++)
            {
                U[] array = new U[HudRendererBatch.batchMaxCount];
