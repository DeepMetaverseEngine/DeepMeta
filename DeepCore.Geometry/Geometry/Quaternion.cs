...
namespace DeepCore.Geometry
{
    ...
    public struct Quaternion : IEquatable<Quaternion>
    {
        ...
        {
            return new Quaternion() { Value = value };
        }
        ...
        {
            this.Value.X = vectorPart.X;
            this.Value.Y = vectorPart.Y;
            this.Value.Z = vectorPart.Z;
            this.Value.W = scalarPart;
        }
        ...
        {
            Quaternion quaternion2 = identity;
        ...
        internal static void ToMatrix(Quaternion quaternion, out Matrix matrix)
        {
            ...
            //   OpenGL
            matrix.Value.M11 = 1.0f - 2.0f * (y2 + z2);
            matrix.Value.M12 = 2.0f * (xy - wz);
            matrix.Value.M13 = 2.0f * (xz + wy);
            matrix.Value.M14 = 0.0f;

            matrix.Value.M21 = 2.0f * (xy + wz);
            matrix.Value.M22 = 1.0f - 2.0f * (x2 + z2);
            matrix.Value.M23 = 2.0f * (yz - wx);
            matrix.Value.M24 = 0.0f;

            matrix.Value.M31 = 2.0f * (xz - wy);
            matrix.Value.M32 = 2.0f * (yz + wx);
            matrix.Value.M33 = 1.0f - 2.0f * (x2 + y2);
            matrix.Value.M34 = 0.0f;

            matrix.Value.M41 = 2.0f * (xz - wy);
            matrix.Value.M42 = 2.0f * (yz + wx);
            matrix.Value.M43 = 1.0f - 2.0f * (x2 + y2);
            matrix.Value.M44 = 0.0f;
