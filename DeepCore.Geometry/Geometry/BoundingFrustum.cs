...
namespace DeepCore.Geometry
{
    ...
    public class BoundingFrustum : IEquatable<BoundingFrustum>
    {
        ...
        public void Contains(in Vector3 point, out ContainmentType result)
        {
            ...
            {
                // TODO: we might want to inline this for performance reasons
                if (PlaneHelper.ClassifyPoint(in point, in this._planes[i]) > 0)
                {
                    result = ContainmentType.Disjoint;
                    return;
                }
        ...
        {
            result = plane.Intersects(in _corners[0]);
            for (int i = 1; i < _corners.Length; i++)
                if (plane.Intersects(in _corners[i]) != result)
            ...
            switch (ctype)
            {
                ...
                case ContainmentType.Intersects:
                    ...
                    foreach (Plane plane in this._planes)
                    {
                        ...
                        float result2;
                        Vector3.Dot(in ray.Direction, in normal, out result2);

                        float result3;
                        Vector3.Dot(in ray.Position, in normal, out result3);
        ...
        private static void IntersectionPoint(in Plane a, in Plane b, in Plane c, out Vector3 result)
        {
            ...
            result.Value.X = (v1.X + v2.X + v3.X) / f;
            result.Value.Y = (v1.Y + v2.Y + v3.Y) / f;
            result.Value.Z = (v1.Z + v2.Z + v3.Z) / f;
        }
