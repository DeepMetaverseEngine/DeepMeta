...
namespace DeepCore.Geometry
{
    ...
    public struct BoundingBox : IEquatable<BoundingBox>
    {
        ...
        {
            get => new Vector3((Min.X + (Max.X - Min.X) / 2f), (Min.Y + (Max.Y - Min.Y) / 2f), (Min.Z + (Max.Z - Min.Z) / 2f));
        }
        ...
        public BoundingBox(Location3D location, Size3D size)
        {
            this.Min = location;
        ...
        {
            result.Min.Value.X = Math.Min(original.Min.X, additional.Min.X);
            result.Min.Value.Y = Math.Min(original.Min.Y, additional.Min.Y);
            result.Min.Value.Z = Math.Min(original.Min.Z, additional.Min.Z);
            result.Max.Value.X = Math.Max(original.Max.X, additional.Max.X);
            result.Max.Value.Y = Math.Max(original.Max.Y, additional.Max.Y);
            result.Max.Value.Z = Math.Max(original.Max.Z, additional.Max.Z);
        }
        ...
        {
            return (obj is BoundingBox box) ? this.Equals(in box) : false;
        }
        ...
        public void Intersects(in Plane plane, out PlaneIntersectionType result)
        {
            ...
            Vector3 positiveVertex = Vector3.Zero;
            Vector3 negativeVertex = Vector3.Zero;
        ...
        public static BoundingBox Expand(in BoundingBox box)
        {
            var box2 = box;
            box2.Min.X = Math.Min(box.Min.X, Math.Min(box.Min.Y, box.Min.Z));
            box2.Min.Y = Math.Min(box.Min.X, Math.Min(box.Min.Y, box.Min.Z));
            box2.Min.Z = Math.Min(box.Min.X, Math.Min(box.Min.Y, box.Min.Z));
            box2.Max.X = Math.Max(box.Max.X, Math.Max(box.Max.Y, box.Max.Z));
            box2.Max.Y = Math.Max(box.Max.X, Math.Max(box.Max.Y, box.Max.Z));
            box2.Max.Z = Math.Max(box.Max.X, Math.Max(box.Max.Y, box.Max.Z));
            return box2;
        }

        public Vector3 Clamp(in Vector3 _pos)
        {
            var pos = _pos;
            if (pos.X < this.Min.X) pos.X = this.Min.X;
            if (pos.Y < this.Min.Y) pos.Y = this.Min.Y;
            if (pos.Z < this.Min.Z) pos.Z = this.Min.Z;
            if (pos.X > this.Max.X) pos.X = this.Max.X;
            if (pos.Y > this.Max.Y) pos.Y = this.Max.Y;
            if (pos.Z > this.Max.Z) pos.Z = this.Max.Z;
            return pos;
        }
    }
    ...
    public struct BoundingBox2D : IEquatable<BoundingBox2D>
    {
        ...
        {
            get => new Vector2((Min.X + (Max.X - Min.X) / 2f), (Min.Y + (Max.Y - Min.Y) / 2f));
        }
        public RectangleF Bounds
        {
            get => new RectangleF(Min, Max - Min);
        }
        ...
        public BoundingBox2D(RectangleF rect)
        {
            this.Min = rect.Location;
            this.Max = rect.Location + rect.Size;
        }
        ...
        public void Contains(in BoundingBox2D box, out ContainmentType result)
        {
            result = Contains(box);
        }

        public ContainmentType Contains(in Vector2 point)
        {
            ContainmentType result;
            this.Contains(in point, out result);
            return result;
        ...
        public void Contains(in Vector2 point, out ContainmentType result)
        {
            //first we get if point is out of box
            if (point.X < this.Min.X
                || point.X > this.Max.X
        ...
        public static void CreateMerged(in BoundingBox2D original, in BoundingBox2D additional, out BoundingBox2D result)
        {
            result.Min.Value.X = Math.Min(original.Min.X, additional.Min.X);
            result.Min.Value.Y = Math.Min(original.Min.Y, additional.Min.Y);
            result.Max.Value.X = Math.Max(original.Max.X, additional.Max.X);
        ...
        public bool Equals(in BoundingBox2D other)
        {
            return (this.Min == other.Min) && (this.Max == other.Max);
        }
        ...
        public override bool Equals(object obj)
        {
            return (obj is BoundingBox2D box) ? this.Equals(in box) : false;
        }
        ...
        public bool Intersects(in BoundingBox2D box)
        {
            bool result;
            Intersects(in box, out result);
            return result;
        ...
        public void Intersects(in BoundingBox2D box, out bool result)
        {
            if ((this.Max.X >= box.Min.X) && (this.Min.X <= box.Max.X))
            {
                if ((this.Max.Y < box.Min.Y) || (this.Min.Y > box.Max.Y))
        ...
        public static bool operator ==(in BoundingBox2D a, in BoundingBox2D b)
        {
            return a.Equals(in b);
        }
