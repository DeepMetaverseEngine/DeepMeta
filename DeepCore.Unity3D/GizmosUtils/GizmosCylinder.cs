...
namespace DeepGame3D.Unity.GizmosUtils
{
    public class GizmosCylinder : MonoBehaviour
    {
        ...
        void OnDrawGizmos()
        {
            ...
            for (float theta = 0; theta < 2 * Mathf.PI; theta += Theta)
            {
            ...
            // 朝向
            var rot = Quaternion.AngleAxis(Direction * Mathf.Rad2Deg, Vector3.up);
            Gizmos.DrawLine(pos, pos + rot * Vector3.right * (Radius * 2));
