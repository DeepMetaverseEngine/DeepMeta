...
namespace DeepMetaGame.Data.Misc
{
    ...
    public class LaunchSpell : ISNData
    {
        ...
        [Desc("如果触发为扇形，则定义扇形范围角度", "散射")]
        [DependOnProperty(nameof(MultiLaunch))]
        public float Angle360 { get => CMath.RadianToAngle(Angle); set { Angle = CMath.AngleToRadian(value); } }
        //----------------------------------------------------------------------------------------
        ...
        [Desc("发射初始角度", "发射")]
        public float StartAngle360 { get => CMath.RadianToAngle(StartAngle); set { StartAngle = CMath.AngleToRadian(value); } }

        [Desc("随机角度", "发射")] public float RandomAngle = 0;
        [Desc("随机角度", "发射")]
        public float RandomAngle360 { get => CMath.RadianToAngle(RandomAngle); set { RandomAngle = CMath.AngleToRadian(value); } }
        ...
        [Desc("法术发射角度（炮口角度）", "炮口"), DependOnProperty(nameof(FromUnitBody), false)]
        public float LaunchSpellAngle360 { get => CMath.RadianToAngle(LaunchSpellAngle); set { LaunchSpellAngle = CMath.AngleToRadian(value); } }
        //----------------------------------------------------------------------------------------
        [DependOnProperty(nameof(ChainLevel))]
        public bool IsChain { get => ChainLevel > 0; }
        [Desc("连锁等级，只在技能中LaunchSpell有效（闪电链类数量，如果Spell launch spell ID一致，则传递此值）如果为0，则不传递相同的Spell。", "连锁")]
        public int ChainLevel = 0;
        [Desc("连锁忽略发起者", "连锁")]
        [DependOnProperty(nameof(IsChain))]
        public bool IgnoreSender;
        [Desc("弹射类子弹忽略弹射本体", "连锁")]
        public bool InheritDamageTargetList = false;
        [Desc("连锁结束时的最终触发", "连锁")]
        public LaunchSpell FinalChainSpell;
        //----------------------------------------------------------------------------------------
        [Desc("重新锁定范围内目标", "锁定")]
        public FocusTarget ResetSeekingTarget;
        [Desc("自动锁定范围内目标", "锁定")]
        public bool IsAutoSeekingTarget = false;
        [Desc("自动锁定范围内目标（范围）", "锁定")]
        [DependOnProperty(nameof(IsAutoSeekingTarget))]
        public float SeekingTargetRange = 10f;
        [Desc("发射这面朝目标", "锁定")]
        [DependOnProperty(nameof(IsAutoSeekingTarget))]
        public bool SenderFaceToTarget = false;
        ...
        [Desc("自动锁定范围内目标（身体）", "锁定")]
        [DependOnProperty(nameof(IsAutoSeekingTarget))]
        public SeekingTargetAnchor SeekingAnchor = SeekingTargetAnchor.Waist;
        //----------------------------------------------------------------------------------------

        [Desc("嵌套法术", "嵌套")]
        public List<LaunchSpell> SubSpells;
